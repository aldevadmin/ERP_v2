...
**Overview · Order Lines · Planning · Fulfilment · Packing · Shipping · Loading · Documents · Activity/History**

"Order Lines" is inserted here, ahead of "Planning" — not in the original tab list above, which assumed lines already existed by the time planning needed them. **Built** as six top-level tabs (`frontend/src/modules/export-orders/ExportOrderDetailPage.tsx`, an AntD `Tabs` shell): `overview`, `lines`, `planning`, `fulfilment`, `packing`, `shipping`, `loading`; `documents`/`activity` are not built yet. **Shipping now comes before Loading** (previously the reverse) — coordinators plan a shipment's SKU allocations in Shipping before they try to log cartons against it in Loading, and the old ordering read as if Loading came first, which was a source of confusion.

**Regrouped three times.** First from an earlier flat 10-tab layout (SKU Planning, Production, Procurement, and the 4 Packing Materials tabs were previously all top-level) once the tab bar got too wide for daily use. Then, as part of the Phase 1 visual/IA redesign, the former **Gathering** tab (nested Production/Procurement sub-tabs) was replaced by **Fulfilment** and the former standalone **Shipments** tab (shipment planning) was folded into **Shipping**. Most recently, the original **Planning** tab (nesting SKU Planning and the 4 Packing Materials tabs) was **deleted outright**, not merged — see the "Planning tab removed" note below — and the tab formerly called **Planning v2** was renamed to **Planning**, becoming the sole planning surface, now that the name is free.
...
**Original Planning tab removed.** The earlier `ExportOrderPlanningTab.tsx` and everything it exclusively depended on — `ExportOrderSkuPlanningTab.tsx` (§5.2), `ExportOrderPackingMaterialsTab.tsx` (§5.5), `SkuPlanningStatusTag.tsx`, `SkuRiskTag.tsx` — were deleted at the user's explicit instruction ("Delete Planning tab completely"), after confirming the functional cost: planned/production/procurement dates, responsible person/team, `planning_status`/`risk_status`, remarks, and (on the Packing Materials sub-tabs) received/accepted quantity, responsible person, status, remarks, and editable ETA all lose their only UI entry point. Existing data stays in the database — nothing was deleted server-side — it's just no longer reachable or settable from the UI. Risk/Status tags elsewhere (Fulfilment/Packing/Loading) that read `risk_status` stay frozen at whatever was last set, or default to On Track. What was **Planning v2** (§5.2a, renamed to **Planning** — `ExportOrderPlanningV2Tab.tsx` renamed to `ExportOrderPlanningTab.tsx`) is unaffected by the deletion and is now the only planning surface in the UI.
...
Was built as `ExportOrderSkuPlanningTab.tsx`, nested inside the original Planning tab. **Deleted** along with the rest of that tab (see the "Original Planning tab removed" note in §5) — no longer reachable from the UI. §5.2a below (the tab now simply called **Planning**) is the surviving planning surface; it does not reproduce this tab's Drawer-based per-SKU editing (Planned Start/Completion Date, Planned Order/Receipt Date, Responsible Person/Team, Status/Risk, Remarks) — those fields have no UI entry point anymore.

### 5.2a Planning (screenshot-driven layout, formerly "Planning v2")

**Built** (`ExportOrderPlanningTab.tsx`, top-level tab `planning`) — a second, denser presentation of the same underlying planning data, requested to match a specific mockup rather than to change what's built. Originally shipped alongside the older Planning tab under the name "Planning v2"; once that older tab was deleted (§5), this one was renamed from **Planning v2** to **Planning**, since the plain name was free and this is now the only planning tab. Two inline-editable tables in one screen, no drawer:
...
### 5.5 Packing Materials (removed)

Was built as `ExportOrderPackingMaterialsTab.tsx`, one component reused across 4 nested sub-tabs (Cartons, Pouches, Retail Stickers, Box Labels) inside the Planning tab — deliberately **not** a full Procurement module: no vendor ledger, no transaction log, V1 allowed direct manual updates (business-rules.md §6).

Each sub-tab opened with a glance-able `Statistic` row (Total Required/Available/Shortage/SKUs Short) and a summary table (SKU, Required, Available Stock, Shortage, To Procure, Ordered, Expected Arrival, Received, Accepted, Responsible, Status), with a `Drawer` to edit Available Stock/To Procure/Ordered/Expected Arrival/Received/Accepted/Responsible Person/Status/Remarks per row. **Deleted** along with the rest of the original Planning tab (see the "Original Planning tab removed" note in §5) — Available Stock, To Procure, Ordered, Expected Arrival, Received, Accepted, Responsible, Status, and Remarks are all still stored on `PackingMaterialRequirement` and readable via the API, but have no UI entry point anymore. The Planning tab's "Packing Material Planning by SKU" table (§5.2a) covers a subset of the same data (Required/Available/To Procure only) — it does not replace Received/Accepted/Responsible/Status/Remarks tracking.
...
**Rebuilt** (`ExportOrderLoadingTab.tsx`, screenshot-driven, replacing the earlier design described below, embedded on the `loading` tab — no standalone screen yet) — optimized for container stuffing day. Loading is scoped to one Shipment at a time (domain-model.md §3.8.1) — a SKU split across Shipments has genuinely separate loading progress per Shipment. The shipment-picker `Select` was briefly removed, then **restored** one round later once real order data surfaced the cost of dropping it: an order with a SKU split across two Shipments had one Shipment's allocation become invisible with no picker to reach it. The picker sits in the readiness card's header, defaulting to the order's first Shipment. An order with no Shipments yet still shows a "create a shipment first" `Empty` state pointing at the Shipping tab instead of the table.

**Order lines not yet on the selected Shipment — shown, not hidden.** A `ShipmentLine` (a SKU's planned allocation onto a Shipment) is still a separate, explicit planning step done in the Shipping tab — that stayed a deliberate, confirmed requirement, not automated away. What changed: the readiness table used to simply omit any order line with no `ShipmentLine` on the selected Shipment, which silently hid real PO line items (confirmed against live order data, not hypothetical — a real order had one of its two SKUs completely missing from Loading). Now every `ExportOrderLine` on the order is cross-referenced against the selected Shipment's lines (`listExportOrderLines` + `listShipmentLines`, combined into one `ReadinessRow[]` client-side); a line with no allocation renders with `—` in every Loadable/Loaded/Balance/Last Update/Progress cell, a grey **"Not Planned"** `Tag` in Status, and an **"Add to Shipment"** action instead of "Update Loading". That action opens `AddToShipmentModal.tsx` — a single Planned Qty (pieces) field (defaulting to the SKU's full `required_pieces`) plus optional Remarks, calling the existing `createShipmentLine` endpoint (unchanged, already used by the Shipping tab) — no new backend surface, just a second entry point into it. The over-allocation error ("Only N of this SKU is still unallocated across shipments," business-rules.md §7) surfaces here the same way it already does in the Shipping tab if the default is too high.
...
**Loading Transactions — new, collapsed by default** (AntD `Collapse`, matches the request literally: "I want it to be collapsed") — the order-wide-per-shipment, paginated activity feed (api-spec.md §6, domain-model.md §3.8.1), same shape as Fulfilment's/Packing's "Recent Transactions" tables but scoped to the selected Shipment only. Independently filterable by SKU. Columns: Date, SKU, Cartons, Pouches, Pieces, Reason, Remarks — no separate "Recorded By" column (this table skews toward "what happened," not "who did it," matching the mockup's stated purpose "just to say what is getting loaded"). Only covers SKUs actually allocated to the selected Shipment — an unplanned line has no transactions to show until it's added via "Add to Shipment" above.

**"Update Loading" modal — rebuilt to a second, screenshot-driven mockup** (`AddLoadingTransactionModal.tsx`, replacing the earlier per-row modal design). Its stated purpose changed the shape of the form: capturing how many cartons were loaded, checked in *real time roughly every 15-30 minutes* — today typed by a coordinator counting boxes, eventually auto-filled by a barcode scan of each carton (domain-model.md §3.8.1's "confirmed, not changed" note) — not a single end-of-shift snapshot. Title is two lines: "Update Loading" (bold) then the SKU code (blue, smaller) directly below it — no `—` separator like the earlier title format. Fields, in a two-column grid (`Row`/`Col`, not the single-column stack every other transaction modal in this app uses — a deliberate visual match to this specific mockup):
- **SKU** / **Loadable Qty** (`planned_qty`, pieces) — both read-only, top row.
- **Already Loaded Qty** (new — `shipmentLine.actual_loaded_qty`, the cumulative *before* this entry, pieces) / **Cartons Loaded Now** (required, this entry's own incremental amount) — second row.
- **Pieces Loaded (Auto)** — full width, a static "0 pcs" placeholder with a caption, no client-side calculation (same restraint as the Packing tab's Pieces Packed field, per CLAUDE.md's "React must not independently implement business calculations").
- **Net Weight (kg) (Auto)** / **Gross Weight (kg) (Auto)** — read-only, showing the shipment line's last-known server-computed cumulative value (`0.00 kg` formatting) with a "Calculated automatically." caption — displaying already-fetched server data isn't a new calculation, so this stays a plain display, not a client business rule.
- **Remarks (Optional)** — textarea.

**Dropped from the earlier design, per this mockup and its stated real-time-entry purpose**:
- **No manual date field.** "Should capture date & time automatically" — the entry's `date` is set to today automatically at submit time (no picker shown); `created_at` (already recorded on every transaction) covers the precise timestamp, feeding the readiness table's existing "Last Update" column.
- **No Pouches Loaded field.** The mockup only shows cartons — matches "capture how many cartons loaded" literally, and fits a scan-each-carton future. `LoadingTransaction.pouches_loaded`/`POUCH_LOADED` remain valid, tested backend capabilities (api-spec.md §6) for direct API use; they're just not reachable from this modal. The Loading Summary card's "Total Pouches Loaded" figure stays in place but will read 0 going forward, since there's no UI path left to log pouches.
- **No Reason field.** Paired with dropping the backend's "reason required whenever the running total doesn't match planned" rule (business-rules.md §7) — under a real-time partial-update workflow, a not-yet-matching total is the normal, expected mid-stream state, not a one-time end-of-day variance needing an explanation. `variance_reason` is still a real, storable field (accepted if a caller supplies one via the API), it just has no UI affordance here anymore.

Button label is **"Save Update"** (not "Save"), matching the mockup.
