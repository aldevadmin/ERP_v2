...
- `pieces_per_pouch` / `pouches_per_carton` are copied onto `ExportOrderLine` from the published `customer_mappings.CustomerProductMappingVersion` resolved via `customer_mappings.resolve_customer_product(customer, customer_sku, order.customer_po_date)` (see `domain-model.md` §3.2) whenever `item` is set for the first time or changed to a different item, and frozen from then on — the resolved version's id is itself recorded on `ExportOrderLine.source_mapping_version` for audit. Resolution is keyed by `customer_sku`, not `item` — a customer can have several simultaneous mappings against the same item (different pack sizes sold as different SKUs), so the mapping's `item` is cross-checked against the line's own `item` and rejected on mismatch. A later change to the master config (even publishing a new mapping or packaging version) never retroactively changes an existing line unless its `item` is itself re-set. **Tested**
- **Validation**: a `Pouch`/`Carton`-unit line requires a resolvable, published, currently-effective Customer Product Mapping for `(order.customer, customer_sku_code)` whose `item` matches the line's `item`, with the needed packing field(s) filled in on its pinned packaging profile version (`pieces_per_pouch` always; `pouches_per_carton` additionally for Carton). If unresolvable, the API rejects the write with `400` attached to `item` (e.g. "No published Customer Product Mapping is effective for this customer, SKU, and order date. Set Pieces per Pouch in Customer Product Mappings first.") — never a silent fallback or a zero-filled computation. `Piece`-unit lines have no such requirement; `required_pieces` is just `original_customer_quantity`. **Tested**
...
- **Packing Material Requirement — Built** (`backend/apps/export_orders/models.py` — `PackingMaterialRequirement`, one row per (`ExportOrderLine`, `material_type`), 4 fixed types: Cartons, Pouches, Retail Stickers, Box Labels — no Silica Gel tracking in V1):
  ...
  - `RETAIL_STICKER` required quantity = `required_pouches` (a fixed 1:1 ratio), and only when the line's snapshotted `has_retail_sticker` flag (resolved from the Customer Product Mapping at line creation) is true — otherwise `0`. **Tested** (`test_export_order_line_model.py`)
  - `BOX_LABEL` has no formula anywhere in the Customer Product Mapping data — its required quantity is manually entered (`manual_required_qty`), the one deliberate exception to "derived, not stored" for this feature. Writing `manual_required_qty` for any other `material_type` is rejected. **Tested** (`test_packing_material_api.py`)
...
- `variance_reason` is **per-transaction** (`LoadingTransaction`, not one value per `ShipmentLine`) and **optional, never required** — **superseded** an interim rule (kept for one round of the Loading tab rebuild) that required it whenever the cumulative total didn't exactly match `planned_cartons`. Dropped once the workflow was clarified: loading is logged in frequent real-time partial updates (every 15-30 minutes, moving toward barcode-scan-driven entries), so a not-yet-matching running total is the *normal* case, not an exception needing an explanation — the old rule would have blocked nearly every save. Over/under vs. plan stays purely informational, surfaced via the readiness table's Balance/Status columns, never blocking a save. Still a **fixed 7-option choice field, not free text**, when a caller does supply one: Container space constraint, Customer approved adjustment, Additional space available, Packing shortage, Product shortage, Weight restriction, Other (the `ShipmentLine.VarianceReason` enum, reused by `LoadingTransaction`). **Tested.**
