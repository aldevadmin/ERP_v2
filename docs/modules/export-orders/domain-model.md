...
| variance_reason | Char, choices, blank, **optional** — same fixed 7-option list as before (Container space constraint, Customer approved adjustment, Additional space available, Packing shortage, Product shortage, Weight restriction, Other), now **per-transaction** instead of one value per `ShipmentLine`, and never required (see below) |
| remarks | Text, blank |
| entered_by | via `created_by` (inherited from `BaseModel`) |

`calculated_pieces` is a computed property, same shape as `PackingTransaction.calculated_pieces` — `cartons_loaded × pieces_per_carton` or `pouches_loaded × pieces_per_pouch` depending on `entry_type`. Exactly one of `cartons_loaded`/`pouches_loaded` may be set per transaction, matching `entry_type` — same mutual-exclusivity rule as `PackingTransaction`. **Tested.** The rebuilt "Update Loading" modal only ever sends `CARTON_LOADED` (`POUCH_LOADED` stays a real, tested backend capability, just not reachable from this UI — see ui-spec.md §8).

**`variance_reason` requirement — built, then removed.** An earlier pass required it whenever the *cumulative* `actual_loaded_cartons` an entry would produce didn't exactly equal `planned_cartons` (adapting the pre-ledger single-snapshot rule to a ledger). That was **superseded** once the workflow was clarified: loading is meant to be logged in frequent real-time partial updates (every 15-30 minutes, eventually barcode-scan-driven, each scan just incrementing the count), so a not-yet-matching running total is normal, not exceptional — the old rule would have demanded an explanation on nearly every save. `variance_reason` is now accepted if supplied but never required; `LoadingTransactionSerializer.validate()` only still enforces the cartons/pouches XOR rule. **Tested** (both that a partial entry saves cleanly without a reason, and that a supplied reason is still stored).

**Order-wide-per-shipment log — Built**: `GET /export-orders/{id}/shipments/{shipment_pk}/loading-transactions/` (api-spec.md §6, "Loading") — scoped to *one Shipment*, unlike Fulfilment's/Packing's order-wide logs: a SKU split across Shipments has genuinely separate loading progress per Shipment, so mixing them into one feed would misrepresent which container a row belongs to. The UI's shipment-picker `Select` was briefly removed then restored (ui-spec.md §8) once a real order's SQ split across two Shipments made the cost of dropping it concrete — one Shipment's allocation was simply invisible with no way to reach it.

**Unplanned order lines surfaced, not hidden — UI-layer only, no model change.** An `ExportOrderLine` with no `ShipmentLine` on the selected Shipment used to be silently absent from the Loading tab's readiness table. Confirmed against a live order (not hypothetical) that this genuinely dropped a real PO line item from view. The fix reads `ExportOrderLine`s and the selected Shipment's `ShipmentLine`s together client-side and renders a row either way — no new backend endpoint (`createShipmentLine`, already used by the Shipping tab, is reused as the "Add to Shipment" action's write path). Allocating a SKU to a Shipment stays a required, explicit step (confirmed, not automated away) — the change is that the *need* to do it is now visible in Loading itself, not just noticed by its absence.

**Confirmed, not changed — per-SKU entry stays right, including for a future barcode-scanner workflow**: each `LoadingTransaction` is scoped to one `ShipmentLine` (one SKU), entered one at a time — considered and explicitly kept this way rather than introducing a multi-SKU "loading session" header, because a future barcode scanner is just a different *input method* for the same per-SKU action (a scan auto-increments `cartons_loaded` instead of a coordinator typing it), not a different data shape. No session/grouping concept needed unless the actual scanning workflow, once built, proves otherwise.
