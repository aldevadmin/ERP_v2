import { afterEach, describe, expect, it, vi } from 'vitest'
import { fireEvent, render, screen, waitFor } from '@testing-library/react'
import NewExportOrderModal from './NewExportOrderModal'
import * as exportOrdersApi from './api'
import * as customersApi from '../customers/api'
import type { CustomerListResponse } from '../customers/types'
import type { ExportOrder } from './types'

vi.mock('./api')
vi.mock('../customers/api')

const mockedApi = vi.mocked(exportOrdersApi)
const mockedCustomersApi = vi.mocked(customersApi)

afterEach(() => {
  vi.clearAllMocks()
})

const customersResponse: CustomerListResponse = {
  count: 1,
  next: null,
  previous: null,
  results: [
    { id: 1, code: 'CUST-1', name: 'Acme Exports', main_poc: '', internal_coordinator: null, internal_coordinator_detail: null, is_active: true },
  ],
}

const created: ExportOrder = {
  id: 1,
  order_number: 'EO-2026-0001',
  customer: 1,
  customer_name: 'Acme Exports',
  customer_po_number: 'PO-100',
  customer_po_date: '2026-01-15',
  export_coordinator: null,
  export_coordinator_detail: null,
  country: '',
  destination_port: '',
  requested_shipment_date: null,
  planned_container_ready_date: null,
  container_type: null,
  currency: '',
  incoterm: '',
  payment_terms: '',
  bill_to: null,
  bill_to_detail: null,
  ship_to: null,
  ship_to_detail: null,
  status: 'PLANNING',
  stage_history: [],
  internal_remarks: '',
  customer_remarks: '',
  po_versions: [],
  created_at: '2026-01-15T00:00:00Z',
  updated_at: '2026-01-15T00:00:00Z',
}

describe('NewExportOrderModal', () => {
  it('creates an export order with the minimal fields', async () => {
    mockedCustomersApi.listCustomers.mockResolvedValue(customersResponse)
    mockedApi.createExportOrder.mockResolvedValue(created)
    const onCreated = vi.fn()

    render(<NewExportOrderModal open onClose={vi.fn()} onCreated={onCreated} />)

    fireEvent.mouseDown(screen.getByRole('combobox'))
    fireEvent.click(await screen.findByText('Acme Exports (CUST-1)'))

    fireEvent.change(screen.getByLabelText('Customer PO Number'), {
      target: { value: 'PO-100' },
    })

    const dateInput = screen.getByPlaceholderText('Select date')
    fireEvent.change(dateInput, { target: { value: '2026-01-15' } })
    fireEvent.keyDown(dateInput, { key: 'Enter', code: 'Enter' })

    fireEvent.click(screen.getByRole('button', { name: 'OK' }))

    await waitFor(() =>
      expect(mockedApi.createExportOrder).toHaveBeenCalledWith({
        customer: 1,
        customer_po_number: 'PO-100',
        customer_po_date: '2026-01-15',
      }),
    )
    await waitFor(() => expect(onCreated).toHaveBeenCalledWith(created))
    expect(mockedApi.uploadPoVersion).not.toHaveBeenCalled()
  })
})
