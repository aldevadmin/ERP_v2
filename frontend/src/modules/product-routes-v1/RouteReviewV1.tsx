...
import { activateRouteVersionV1 } from './api'
import type { ProcessRouteV1, ProductRouteV1WizardStepKey } from './types'

const { Text } = Typography

export default function RouteReviewV1({
  route,
  onActivated,
  onEditStep,
}: {
  route: ProcessRouteV1
  onActivated: (status: string) => void
  onEditStep: (step: ProductRouteV1WizardStepKey) => void
}) {
  ...
  const handleActivate = async () => {
    ...
    setError(null)
    try {
      const result = await activateRouteVersionV1(route.version_id)
  ...
  const targetItemCount = new Set(route.item_mappings.map((m) => m.target_item)).size
  const totalRoles = orderedNodes.reduce((sum, n) => sum + n.inputs.length + n.outputs.length, 0)
  const mappedCount = route.item_mappings.length
  const expectedCount = totalRoles * targetItemCount
  ...
  return (
    <>
      ...
      <Flex gap={12} style={{ marginBottom: 20 }}>
        <Text type="secondary">Item Group: {route.item_group_name}</Text>
            ...
            <div key={node.id} style={{ marginTop: 8, marginLeft: 16 }}>
              {node.outputs.map((output) => {
                ...
                return (
                  <div key={output.id}>
                    <Text type="secondary">
                      ├─ {output.item_group_name} → {destination}
                    </Text>
                  </div>
                )
              })}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <Text>
          {targetItemCount === 0
            ? 'No target items registered yet.'
            : `${targetItemCount} target item${targetItemCount === 1 ? '' : 's'} registered, ${mappedCount}/${expectedCount} role${expectedCount === 1 ? '' : 's'} mapped.`}
        </Text>
      </div>

      {isActive && (
        <Alert type="success" showIcon style={{ marginBottom: 16 }} title="This route is active." />
      )}
      ...
      <Flex justify="space-between">
        <Flex gap={8}>
          ...
          <Button onClick={() => onEditStep('item_mappings')}>Edit Item Mappings</Button>
        </Flex>
        {!isActive && (
