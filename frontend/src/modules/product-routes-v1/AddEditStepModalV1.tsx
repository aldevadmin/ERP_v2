...
import { listProcessesV1 } from '../processes-v1/api'
import type { ProcessV1 } from '../processes-v1/types'
import type { RouteNodeV1, RouteNodeV1FormValues } from './types'

export default function AddEditStepModalV1({
  open,
  node,
  onClose,
  onSave,
  ...
  open: boolean
  node: RouteNodeV1 | null
  onClose: () => void
  onSave: (values: RouteNodeV1FormValues) => Promise<void>
}) {
  const [form] = Form.useForm<RouteNodeV1FormValues>()
  const [processes, setProcesses] = useState<ProcessV1[]>([])
  const [submitting, setSubmitting] = useState(false)

  useEffect(() => {
    if (!open) return
    listProcessesV1({ isActive: true }).then((response) => setProcesses(response.results))
  }, [open])
  ...
  const handleSubmit = async (values: RouteNodeV1FormValues) => {
    setSubmitting(true)
    try {
  ...
  return (
    <Modal
      ...
      destroyOnHidden
    >
      <Form<RouteNodeV1FormValues> form={form} layout="vertical" onFinish={handleSubmit}>
        <Form.Item
          label="Process"
