...
import { saveRouteEdgesV1 } from './api'
import type {
  ProcessRouteVersionV1,
  RouteEdgeV1,
  RouteEdgeV1FormValues,
  RouteEdgeDispositionV1,
  RouteNodeOutputV1,
  RouteNodeV1,
} from './types'
...
interface OutputRowState {
  disposition_type: RouteEdgeDispositionV1 | undefined
...
export default function RouteOutputRoutingFormV1({
  versionId,
  nodes,
  edges,
  onSaved,
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const linearEdges: RouteEdgeV1FormValues[] = edges
      ...
      const branchingEdges: RouteEdgeV1FormValues[] = []
      for (const node of branchingNodes) {
        for (const output of node.outputs) {
      ...
      const result = await saveRouteEdgesV1(versionId, { edges: [...linearEdges, ...branchingEdges] })
      onSaved(result)
      onContinue()
    } catch (err) {
    ...
    <>
      ...
      {branchingNodes.map((node) => (
        <div key={node.id} style={{ marginBottom: 32 }}>
          ...
          {node.outputs.map((output: RouteNodeOutputV1) => {
            const row = rows[keyFor(node.id, output.id)] ?? {
            ...
            return (
              ...
              >
                <Text strong style={{ display: 'block', marginBottom: 8 }}>
                  Output: {output.item_group_name}
                  {output.classification_name && ` (${output.classification_name})`}
                </Text>
                <Flex gap={12} wrap="wrap">
                  <div>
                    ...
                    <Select
                      aria-label={`Next action for ${output.item_group_name}`}
                      style={{ width: 200 }}
                      placeholder="Select an action"
                      value={row.disposition_type}
                      onChange={(value: RouteEdgeDispositionV1) =>
                  ...
                  {row.disposition_type === 'CONTINUE_TO_PROCESS' && (
                    <div>
                      ...
                      <Select
                        aria-label={`Next process for ${output.item_group_name}`}
                        style={{ width: 220 }}
                  ...
                  {row.disposition_type === 'MOVE_TO_STORAGE' && (
                    <div>
                      ...
                      <Select
                        aria-label={`Destination for ${output.item_group_name}`}
                        style={{ width: 220 }}
