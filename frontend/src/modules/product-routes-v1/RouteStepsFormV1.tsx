...
import AddEditStepModalV1 from './AddEditStepModalV1'
import { saveRouteNodesV1 } from './api'
import type { ProcessRouteVersionV1, RouteNodeV1, RouteNodeV1FormValues } from './types'

const { Text } = Typography

function toFormValues(rows: RouteNodeV1[]): RouteNodeV1FormValues[] {
  return rows.map((row) => ({
    id: row.id,
...
export default function RouteStepsFormV1({
  itemGroupName,
  versionId,
  nodes,
  onSaved,
  ...
  const [editingNode, setEditingNode] = useState<RouteNodeV1 | 'new' | null>(null)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const persist = async (nextNodes: RouteNodeV1FormValues[]) => {
    setSaving(true)
    setError(null)
    try {
      const result = await saveRouteNodesV1(versionId, { nodes: nextNodes })
      onSaved(result)
    } catch (err) {
  ...
  const handleSaveNode = async (values: RouteNodeV1FormValues) => {
  ...
  const handleDelete = (node: RouteNodeV1) => {
  ...
  return (
    <>
      <Text strong style={{ display: 'block', marginBottom: 20, fontSize: 16 }}>
        How is the &quot;{itemGroupName}&quot; family processed?
      </Text>
