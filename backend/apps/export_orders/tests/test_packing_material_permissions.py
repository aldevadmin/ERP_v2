...
def _list_url(order: ExportOrder) -> str:
    return f"/api/v1/export-orders/{order.id}/packing-material-requirements/?material_type=CARTON"


def _detail_url(order: ExportOrder, line: ExportOrderLine) -> str:
    return f"/api/v1/export-orders/{order.id}/lines/{line.id}/packing-material-requirements/CARTON/"


@pytest.mark.parametrize("role", INTERNAL_ROLES)
def test_every_internal_role_can_list(role, order, line):
    client = _client_as(role, f"list-{role}")

    response = client.get(_list_url(order))
...
def test_every_internal_role_can_get_singleton(role, order, line):
    ...
    response = client.get(_detail_url(order, line))

    assert response.status_code == 200


def test_customer_role_cannot_list(order, line):
...
def test_anonymous_cannot_list(order, line):
    client = APIClient()
...
def test_can_manage_roles_can_patch(role, order, line):
    ...
    response = client.patch(_detail_url(order, line), {"available_stock": 100}, format="json")
...
def test_customer_role_cannot_patch(order, line):
    client = _client_as("Customer", "customer-patch")
