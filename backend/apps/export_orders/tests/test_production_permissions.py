...
CAN_MANAGE_ROLES = ["Export Coordinator", "Production Coordinator", "Manager/Admin"]
...
def supply_plan(line):
    return SKUSupplyPlan.objects.create(export_order_line=line, quantity_to_produce=10_000)
...
def test_can_manage_roles_can_post_transaction(role, order, line):
    client = _client_as(role, f"post-{role}")

    response = client.post(_transactions_url(order, line), _post_body(), format="json")

    assert response.status_code == 201


@pytest.mark.parametrize("role", CANNOT_MANAGE_ROLES)
def test_other_internal_roles_cannot_post_transaction(role, order, line):
...
def test_customer_role_cannot_post_transaction(order, line):
...
def test_anonymous_cannot_post_transaction(order, line):
    client = APIClient()
