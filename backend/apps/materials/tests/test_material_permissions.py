...
def test_can_manage_roles_can_create(role):
    ...
    response = client.post(
        "/api/v1/materials/",
        {"code": f"NEW-{role}", "name": "Raw Leaf", "unit": "Kg"},
        format="json",
    )
...
def test_other_internal_roles_cannot_create(role):
    ...
    response = client.post(
        "/api/v1/materials/",
        {"code": f"NC-{role}", "name": "Raw Leaf", "unit": "Kg"},
        format="json",
    )
...
def test_other_internal_roles_cannot_update(role, organization):
    material = Material.objects.create(
        code=f"UPD-{role}", name="Raw Leaf", unit="Kg", organization=organization
    )
    client = _client_as(role, f"noupdate-{role}")

    response = client.patch(f"/api/v1/materials/{material.id}/", {"name": "Renamed"}, format="json")
