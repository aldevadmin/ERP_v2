...
def test_can_manage_roles_can_create(role, organization):
    ...
    response = client.post(
        "/api/v1/processes/",
        {"name": f"NEW-{role}", "category": category.id, "resource_type": "STATION"},
        format="json",
    )
...
def test_other_internal_roles_cannot_create(role, organization):
    ...
    response = client.post(
        "/api/v1/processes/",
        {"name": f"NC-{role}", "category": category.id, "resource_type": "STATION"},
        format="json",
    )
...
def test_other_internal_roles_cannot_update(role, organization):
    ...
    response = client.patch(f"/api/v1/processes/{process.id}/", {"name": "Renamed"}, format="json")
