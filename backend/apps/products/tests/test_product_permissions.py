...
def test_can_manage_roles_can_create(role):
    ...
    response = client.post(
        "/api/v1/products/",
        {"sku_code": f"NEW-{role}", "name": "Areca Plate", "base_unit": "Piece"},
        format="json",
    )
...
def test_other_internal_roles_cannot_create(role):
    ...
    response = client.post(
        "/api/v1/products/",
        {"sku_code": f"NC-{role}", "name": "Areca Plate", "base_unit": "Piece"},
        format="json",
    )
...
def test_other_internal_roles_cannot_update(role, organization):
    product = Product.objects.create(
        sku_code=f"UPD-{role}", name="Areca Plate", base_unit="Piece", organization=organization
    )
    client = _client_as(role, f"noupdate-{role}")

    response = client.patch(
        f"/api/v1/products/{product.id}/", {"name": "Renamed"}, format="json"
    )
