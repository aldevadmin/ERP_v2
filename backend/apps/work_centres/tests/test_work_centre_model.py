...
def test_code_must_be_unique(organization):
    ...
    with pytest.raises(IntegrityError), transaction.atomic():
        WorkCentre.objects.create(
            code="WC-1", name="Duplicate", type=WorkCentre.Type.MACHINE, organization=organization
        )


def test_str_includes_code(organization):
    work_centre = WorkCentre.objects.create(
        code="WC-2", name="Press 01", type=WorkCentre.Type.MACHINE, organization=organization
    )
...
def test_capability_unique_per_work_centre_and_process(organization):
    work_centre = WorkCentre.objects.create(
        code="WC-3", name="Press 02", type=WorkCentre.Type.MACHINE, organization=organization
    )
